(function(){
	...
	var plugin = function(){
		...
		AtKit.addLocalisationMap("se", {
			...
			"css_changePage": "Ändra sidans utseende",
			"css_changeATbar": "Ändra ATbars färger",
			"css_changeBackground" : "Bakgrundsfärg",
			"css_set" : "Välj",
			"css_black": "Svart",
			"css_white": "Vit",
			"css_grey": "Grå",
			"css_random": "Slumpvis vald",
			"css_reset_defaults" : "Återställ",
			"css_change_linktext" : "Ändra färg på text och länkar",
			"css_textcolour": "Textfärg:",
			"css_colour_original": "Orginal",
			"css_colour_red": "Röd",
			"css_colour_blue": "Blå",
			"css_colour_green": "Grön",
			"css_colour_yellow": "Gul",
			"css_colour_orange": "Orange",
			"css_linkColour":"Länkfärg:",
			"css_apply": "Ok",
			"css_change_page": "Ändra sidans utseende",
			"css_black_white": "Svart på vitt",
			"css_white_black": "Vitt på svart",
			"css_yellow_black": "Gult på svart",
			"css_black_yellow": "Svart på gult",
			"css_white_grey": "Vitt på grått"
		});
		...
		var CSSDialogs = {
			...
			"toolbar": {
				...
				"body":"<label for=\"sbbackgroundcolour\">" + AtKit.localisation("css_changeBackground") + "</label><input type=\"text\" name=\"sbbackgroundcolour\" id=\"sbbackgroundcolour\"> <button id=\"sbSetColour\">" + AtKit.localisation("css_set") + "</button> <br /> <p><button onclick=\"document.getElementById('sbbackgroundcolour').value = \'" + AtKit.localisation("css_black") + "\'\">" + AtKit.localisation("css_black") + "</button> <button onclick=\"document.getElementById('sbbackgroundcolour').value = \'" + AtKit.localisation("css_white") + "\'\">" + AtKit.localisation("css_white") + "</button> <button onclick=\"document.getElementById('sbbackgroundcolour').value = \'" + AtKit.localisation("css_grey") + "\'\">" + AtKit.localisation("css_grey") + "</button></p> <br /> <button id=\"sbRandomColour\"> " + AtKit.localisation("css_random") + "</button> <button id=\"sbColourReset\">" + AtKit.localisation("css_reset_defaults") + "</button>"
			},
			"siteColours": {
		...
		CSSFunctions = {
			"changeToolbar": function(){
				...
				$lib('#sbSetColour').click(function(){ if(AtKit.getLanguage()==="se") {
						var sweColour = $lib("#sbbackgroundcolour").val().toString();
						sweColour=sweColour.toLowerCase();
						if(sweColour==="svart") {
							sweColour="black";
						}
						else if(sweColour==="vit") {
							sweColour="white";
						}
						else if(sweColour==="grå") {
							sweColour="grey";
						}
						else if(sweColour==="grön") {
							sweColour="green";
						}
						else if(sweColour==="röd") {
							sweColour="red";
						}
						else if(sweColour==="gul") {
							sweColour="yellow";
						}
						else if(sweColour==="blå") {
							sweColour="blue";
						}					
						else if(sweColour==="brun") {
							sweColour="brown";
						}
						else if(sweColour==="rosa") {
							sweColour="pink";
						}
						else if(sweColour==="lila") {
							sweColour="purple";
						}
						AtKit.call('setColour', sweColour );
					}
					else {
