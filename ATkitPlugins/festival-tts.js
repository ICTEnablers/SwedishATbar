  ...
  var TTS = function(defaultLanguage) {
    ...
    self.urls = function(txt, lang) {
      ...
      for (var i=0; i<slices.length; ++i) {
        var slice = slices[i];
        urls.push(
          'https://core.atbar.se/festivaltts.php?t=' + encodeURIComponent(slice)
        );
      }
     ...
     * @param selStr {String} the input text.
     * @param maxSliceLength {Integer} maximum length of each slice.
     * @return {Array} list of slices.
     * @private
     */
    self._sliceInput = function(selStr, maxSliceLength) {
      var selArray = [];

      if(selStr.length>99) {
		var arrayIndex = 0;
		while(selStr.length>99) {
			//Find ,.!?;: or space
			var brk = 99;
			while(brk>0) {
				if(selStr.charAt(brk)==' ' || selStr.charAt(brk)=='.' || selStr.charAt(brk)==',' || selStr.charAt(brk)=='!' || selStr.charAt(brk)=='?' || selStr.charAt(brk)==':' || selStr.charAt(brk)==';') {
					var s = selStr.slice(0,brk);
					selArray.push(s);
					selStr=selStr.slice(brk+1,selStr.length);
					break;
				}
				brk--;
			}
    ...
    self.play = function(url, cb) {
      try {
        (window.soundManager.createSound({
          id: 'festivaltts-' + self._unique_instance_id + '-' + (++self._soundId),
          url: url,
          onfinish: cb
        })).play();
      } catch (err) {
