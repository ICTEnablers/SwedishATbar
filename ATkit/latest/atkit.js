	...
	var AtKit = (function() { 
		...
		function broadcastLoaded(){
			...
			if (typeof window.AtKitLoaded != "undefined") {
				 console.log("broadcast:"+AtKit.internal.__version); 
		...
		API.addScript = function(url, callback){
			...
			//url="http://plugins.atbar.se/resize.js,http://plugins.atbar.se/findEl.js";
		...
		API.importPlugins = function(plugins, callback){
			...
			console.log("pluginsString: "+pluginString);
			var pluginArray = pluginString.split(",");
			var i = 0;
			while(i<pluginArray.length) {
				API.addScript(AtKit.internal.__pluginURL + pluginArray[i]/*pluginString*/ + ".js", callback);
				i++;
			}
		}		
		...
		API.addPlugin = function(identifier){
			console.log("addPlugin: "+identifier);
